...
const intentPatterns: IntentPattern[] = [
  {
    triggers: ['make an app', 'create an app', 'build an app', 'develop an app', 'app builder', 'make app', 'create app', 'build app'],
    intent: 'app_creation',
    toolCategories: ['web development', 'development', 'coding', 'programming'],
    priorityTools: ['lovable.dev', 'bolt.new', 'vercel v0', 'webflow', 'framer', 'builder.io', 'replit', 'github copilot'],
...
export const detectUserIntent = (searchTerm: string): IntentPattern | null => {
  ...
  for (const pattern of intentPatterns) {
    if (pattern.triggers.some(trigger => lowerTerm.includes(trigger))) {
      console.log(`🎯 INTENT DETECTED: ${pattern.intent} for search: "${searchTerm}"`);
      return pattern;
    }
  }
...
export const scoreToolByIntent = (tool: Tool, intent: IntentPattern, searchTerm: string): number => {
  ...
  for (const priorityTool of intent.priorityTools) {
    if (tool.title.toLowerCase().includes(priorityTool.toLowerCase())) {
      score += 15000;
      console.log(`🚀 PRIORITY TOOL MATCH: ${tool.title} gets 15000 points for intent ${intent.intent}`);
      break;
    }
  }
  ...
  for (const category of intent.toolCategories) {
    if (tool.category?.toLowerCase().includes(category.toLowerCase())) {
      score += 10000;
      console.log(`📂 CATEGORY MATCH: ${tool.title} gets 10000 points for category ${category}`);
      break;
    }
  }
  ...
  for (const trigger of intent.triggers) {
    if (searchableText.includes(trigger.toLowerCase())) {
      score += 5000;
      console.log(`🔍 INTENT KEYWORD MATCH: ${tool.title} gets 5000 points for ${trigger}`);
      break;
    }
  }
